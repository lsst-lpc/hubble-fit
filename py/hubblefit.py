...
def comp_rms(residuals, dof, err=True, variance=None):
    ...
    if variance is None:                # RMS
        ...
        rms = float(
            N.sqrt(N.sum((residuals**2) / variance) / N.sum(1. / variance)))
        #rms_err = float(N.sqrt(1./N.sum(1./variance)))
        rms_err = N.sqrt(2. * len(residuals)) / \
            (2 * N.sum(1. / variance) * rms)
    if err:
        return rms, rms_err
    return rms
...
def dL_z(zcmb, zhel, omgM):
    ...
    """
    mu_zz = 5 * log10((1 + zcmb) * clight *
                      (quad(intfun, 0, zcmb, args=(omgM))[0] / (10 * H)))
...
def mu_cov(alpha, beta, IDJLA):
    ...
    Ceta = sum([pyfits.getdata(mat)
                for mat in glob.glob('data/covmat/C*.fits')])
    ...
    Cmu[N.diag_indices_from(Cmu)] += sigma[:, 0] ** 2 + \
        sigma[:, 1] ** 2 + sigma_pecvel ** 2
...
class Chi2:  # Class definition
    ...
    def chi2(self, omgM, alpha, beta, Mb, delta_M):
        ...
        mu_z = muexp(self.mB, self.X1, self.C, alpha,
                     beta, Mb, delta_M, self.M_stell)
...
def Hubble_diagram(zcmb, zhel, mB, dmB, X1, dX1, C, dC, M_stell, IDJLA, exp, results, label='Hubble diagram', DisToCenter='', omgM=0.295, alpha=0.141, beta=3.101, Mb=-19.05, delta_M=-0.070, plot=''):
    ...
    m = Minuit(chi2mini.chi2, omgM=0.295, alpha=0.141, beta=3.101, Mb=-19.05, delta_M=-0.070, limit_omgM=(0.2, 0.4), limit_alpha=(0.1, 0.2), limit_beta=(2.0, 4.0),
    ...
    if plot == '':
        f, (P1, P2) = P.subplots(2, sharex=True, sharey=False,
        ...
        P1.set_ylabel(
            '$\\mu = m_b^{*} - (M_{B} - \\alpha X_1 + \\beta C)$', fontsize=20)
        P2.set_xscale('log')
        P2.set_ylabel('$\\mu - \\mu_{\\Lambda {\\rm CDM}}$', fontsize=20)
        P.yticks(fontsize=12)
        dz = 0
        P1.errorbar(zcmb, mufinal, linestyle='', xerr=dz,
                    yerr=dmufinal, ecolor='blue', alpha=1.0, zorder=0)
        P1.set_xscale('log')
        P1.set_ylim(30, 48)
        P1.plot(xfunc, yfunc)
        P1.legend(bbox_to_anchor=(0.5, 1.), fontsize=10)
        P2.scatter(zcmb, ecarts5, c='black', s=2)
        P2.errorbar(zcmb, ecarts5, linestyle='', xerr=dz,
                    yerr=dmufinal, ecolor='blue', alpha=1.0, zorder=0)
    ...
    if DisToCenter != '':
        ...
        dist, (P1, P2) = P.subplots(1, 2, sharex=False,
                                    sharey=True, gridspec_kw=dict(width_ratios=[3, 1]))
        P1.scatter(DisToCenter, ecarts5, c='black', s=2)
        P1.errorbar(DisToCenter, ecarts5, linestyle='', xerr=dz,
                    yerr=dmufinal, ecolor='blue', alpha=1.0, zorder=0)
        P1.set_xscale('log')
...
if __name__ == '__main__':
    ...
    # file used to write some results (use the one you want instead of mine).
    results = open('./Hubble.txt', 'w')
    ...
    results.write(
        '#Chi2/d.o.f. Chi2 mean_of_the_residuals rms Omega_M  alpha beta Mb delta_M \n')
    fit = Hubble_diagram(zcmb, zhel, mB, dmB, X1, dX1, C, dC, M_stell, IDJLA, exp, results,
                         'Hubble diagram, made with JLA dataset,', '', omgM, alpha, beta, Mb, delta_M)
    results.close()
    if False:
        P.show()
    pass
